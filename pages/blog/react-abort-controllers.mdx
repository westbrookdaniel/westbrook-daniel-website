...
In react we can use fetch in a useEffect to get a request when the page loads. So why is the code below a problem?
...
What does this mean? `update on an unmounted component` summarizes it well. We are
change the state of a component that no longer exists because it the component has been removed.
This can lead to your app breaking in weird ways, as well as you wasting resources on unnecessary
actions. So let's fix it by canceling the fetch.
...
This uses the web api `AbortController` as the signal for fetch. By returning a function from `useEffect` we
...
The `AbortSignal` (`controller.singal`) is then passed into the fetch as an argument and voilà!
...
`AbortError` which is getting logged in our `catch`.
...
You can see [here](https://caniuse.com/abortcontroller) that it has 92% support across browsers (although not IE).

Although, this request this still doesn't expose a loading state or indicate to the ui if it has failed.
